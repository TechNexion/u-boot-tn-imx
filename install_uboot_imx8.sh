...
install_uboot_dtb() {
	...
	cd ${TWD}
	if [ "${SOC_DIR}" == "iMX93" ] || [ "${SOC_DIR}" == "iMX91" ] || [ "${SOC_DIR}" == "iMX95" ] ; then
		copy_file ${UBOOT_OUT_DIR}/u-boot.bin ${TWD}/${MKIMAGE_DIR}/${SOC_DIR}/
	else
		copy_file ${UBOOT_OUT_DIR}/u-boot-nodtb.bin ${TWD}/${MKIMAGE_DIR}/${SOC_DIR}/
	fi

	#Copy SPL binary
	cd ${TWD}
	copy_file ${UBOOT_OUT_DIR}/spl/u-boot-spl.bin ${TWD}/${MKIMAGE_DIR}/${SOC_DIR}/

	#Copy device tree file
	cd ${TWD}
	for DTB in ${DTBS}
	do
		copy_file ${UBOOT_OUT_DIR}/arch/arm/dts/${DTB} ${TWD}/${MKIMAGE_DIR}/${SOC_DIR}/
	done
}
...
u_boot_build() {
	...
	run_cmd "Build imx-boot iMX95 config" "yes" "make O=${UBOOT_OUT_DIR} ${kernel_conf_name}"
	run_cmd "Build imx-boot iMX95"        "yes" "make O=${UBOOT_OUT_DIR} -j$(nproc)"
	run_cmd "Build imx-boot iMX95"        "no"  "make O=${UBOOT_OUT_DIR} savedefconfig"
...
generate_imx_boot() {
	...
	copy_file ${UBOOT_OUT_DIR}/tools/mkimage ${TWD}/${MKIMAGE_DIR}/${SOC_DIR}/mkimage_uboot
