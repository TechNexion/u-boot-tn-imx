...
#if defined(CONFIG_TARGET_SBC_IMX6DL)
		//SBC eMMC always exist, no need to detect.
		ret = true;
#else
...
int board_init_pmic(void) {
	...
	if (p && !pmic_probe(p)) {
		...
		pmic_reg_write(p, PFUZE100_VGEN2VOL, reg);
                ...
                /* Set VGENi3 to 2.5V and enable */
                pmic_reg_read(p, PFUZE100_VGEN3VOL, &reg);
                reg &= ~(LDO_VOL_MASK);
                reg |= (LDOB_2_50V | (1 << (LDO_EN)));
...
int board_late_init(void)
{
	if (is_cpu_type(MXC_CPU_MX6DL)) {
		setup_i2c(I2C_PMIC_BUS, CONFIG_SYS_I2C_SPEED, 0x7f, &mx6dl_i2c3_pad_info);
...
{
	if (cpu_is_pop())
		printf("Board: SBC-iMX6-POP\n");
	else
		printf("Board: SBC-iMX6\n");
