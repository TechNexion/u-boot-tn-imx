...
#define MIPI_DSI_I2C_BUS 0
#define MIPI_DSI_LVDS_I2C_BUS 2
...
static const struct mipi_panel_id mipi_panel_mapping[] = {
	{DISPLAY_NAME_MIPI2HDMI, 0, "mipi2hdmi"},
	{DISPLAY_NAME_MIPI5, 0x54, "mipi5"},
	{DISPLAY_NAME_MIPI8, 0x58, "mipi8"},
	{DISPLAY_NAME_MIPI10, 0x59, "mipi10"},
	{DISPLAY_NAME_MIPI2LVDS10, 0, "lvds10"},
	{DISPLAY_NAME_MIPI2LVDS15, 0, "lvds15"},
	{DISPLAY_NAME_MIPI2LVDS21, 0, "lvds21"},
};
...
#define DSI1_BL_EN_PAD IMX_GPIO_NR(1, 13)
#define DSI1_BL_PWM_PAD IMX_GPIO_NR(5, 5)
...
{
	char *fdt_file, *dtoverlay, str_dtov[32];
	...
	if ((fdt_file && !strcmp(fdt_file, "undefined")) || panel) {
		...
		for (i = 0; i < display_count; i++) {
			...
			if ((!panel && dev->detect && dev->detect(dev) && (detected == 0)) ||
					(!strcmp(panel, dev->mode.name))) {
				if (!dtoverlay) {
					dtoverlay = str_dtov;
					strcpy(dtoverlay, mipi_panel_mapping[i].dtbo_name);
				} else {
					if (!strstr(dtoverlay, mipi_panel_mapping[i].dtbo_name)) {
						strcat(dtoverlay, " ");
						strcat(dtoverlay, mipi_panel_mapping[i].dtbo_name);
					}
				}
				detected = 1;
			} else {
				char *match = dtoverlay;
				int len = strlen(mipi_panel_mapping[i].dtbo_name);
				while ((match = strstr(match, mipi_panel_mapping[i].dtbo_name))) {
					*match = '\0';
					strcat(dtoverlay, match + len);
					match++;
				}
			}
		}
